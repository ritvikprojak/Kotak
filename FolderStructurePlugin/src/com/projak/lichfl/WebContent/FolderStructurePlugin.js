require([ "dojo/_base/declare", "dojo/_base/lang", "dojo/_base/array", "dojo/aspect",
	...
	//to handle the properties
		...
		aspect.after(desktop, "_logonCompleted", lang.hitch(this, function() {

			console.log("Desktop._logonCompleted.entry()");
			//default actions
			/*
			 * Open,Preview,Merge and Split,Properties,Create New,View Child Documents,View Notes,Download,Add to Favorites,
			Remove from Favorites,Export,Enable Sync,Disable Sync,Delete,
			Check Out,Check In,Cancel Check Out,Box,Link,Send Email,Version,Folders,Print,Workflow,Holds
			*/
			
			var params = new Object();

			ecm.model.Request.invokePluginService("FolderStructurePlugin",
					"CustomDocumentMenuService", {
						requestParams : params,
						requestCompleteCallback : function(response) {

							if (response) {
		
								_self.configurableMenu = response;
		...
		aspect.after(desktop, "loadMenuActions", lang.hitch(this, function() {

			console.log("Desktop.loadMenuActions.entry()");
			if(!this.defaultMenu){
				this.defaultMenu = ecm.model.desktop.actionLists.ItemContextMenu;
			}
			
			console.log("DefaultMenu:",this.defaultMenu);
			console.log("DefaultMenu:",ecm.model.desktop.actionLists.ItemContextMenu);
			ecm.model.desktop.actionLists.ItemContextMenu = this.defaultMenu;
			debugger;
			var customList;
			if(_self.configurableMenu == undefined){
				
			}else{
				var adminUsers = _self.configurableMenu.adminUsers;
				var adminMenu = _self.configurableMenu.adminMenu;
				var usersMenu = _self.configurableMenu.userMenu;
				var userId = ecm.model.desktop.userId;
				
				if(adminUsers.includes(userId)){
					customList = adminMenu;
				}else{
					customList = usersMenu;
				}
				if(customList!=undefined||customList!=null){
					
					//var customList = _self.configurableMenu.configurableMenu;//["Open","Check In","Cancel Check Out"];
					
					ecm.model.desktop.actionLists.ItemContextMenu = ecm.model.desktop.actionLists.ItemContextMenu.filter(function(x){
					    return customList.includes(x.name);
					});
				}
