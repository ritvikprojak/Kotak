	...
	function AESEncryptCtr(plainText) {
		...
		var plaintext = plainText.text;
		var password =  plainText.key;
		var nBits = plainText.nBits;
	 ...
	 *	 - cipherblock = plaintext xor outputblock
	 */
	function AESDecryptCtr(cipherText) {
		
		var ciphertext = cipherText.text;
		var password = cipherText.key;
		var nBits = cipherText.nBits;
	  if (!(nBits==128 || nBits==192 || nBits==256)) return '';	 // standard allows 128/192/256 bit keys
